...
export default function NewsCard({ item, onVideoPlay }: Props) {
    ...
    <div className="relative aspect-video overflow-hidden bg-[#050810]">
      {item.thumbnail ? (
        ...
        <div className={`w-full h-full bg-gradient-to-br ${placeholderBg} flex items-center justify-center`}>
          <span className="text-2xl font-bold tracking-widest text-white/10" style={{ fontFamily: "Orbitron, monospace" }}>
            {cfg.shortLabel}
          </span>
        </div>
      )}
      ...
      {isYouTube && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/30 transition-colors">
          <div className="w-10 h-10 rounded-full bg-red-600/80 flex items-center justify-center backdrop-blur-sm group-hover:bg-red-600 group-hover:scale-110 transition-all">
    ...
    <div className="flex flex-col flex-1 p-4 gap-2">
      ...
      <div className="mt-auto pt-2 flex items-center justify-between gap-2 border-t border-[#1a2332]">
        <div className="flex items-center gap-2 min-w-0">
          {item.author && <span className="text-xs text-slate-600 truncate">{item.author}</span>}
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <>
              {item.meta.score !== undefined && (
                <span className="flex items-center gap-0.5 text-xs text-slate-600">
                  ...
                  {item.meta.score >= 1000 ? `${(item.meta.score / 1000).toFixed(1)}k` : item.meta.score}
                </span>
              )}
          ...
          <span className="text-xs text-slate-600">{relativeDate(item.publishedAt)}</span>
  ...
  const sharedClass = `group flex flex-col rounded-lg border border-[#1a2332] bg-[#0a0f1a] overflow-hidden transition-all duration-300 hover:border-opacity-70 hover:-translate-y-0.5 hover:shadow-lg`;

  // YouTube: click plays in-page modal, no external link
  if (isYouTube && onVideoPlay) {
    return (
      <div
        className={`${sharedClass} cursor-pointer`}
        style={{ ["--tw-shadow-color" as string]: cfg.glowColor }}
        onClick={() => onVideoPlay(item.meta!.videoId!, item.title)}
      >
        {thumbnail}
        {content}
      </div>
